...
class DynamicNaiveBayesianClassifier(initialEdge: LearnedDiscreteEdge,
  ...
  def inferMostLikelyHiddenStates(observedStates: Seq[ObservedState]): List[String] = {
    ...
    viterbiCompute(probs, observedStates).InferredHiddenStates
  }

  /**
    * Returns log-probability that the sequence of observed states were generated by the DNBC
  ...
  def score(observedStates: Seq[ObservedState]): Double = {
    checkValidObservations(observedStates)
    val probs = viterbiInitialize(observedStates)
    viterbiCompute(probs, observedStates).LogScore
  }
  ...
  private class ViterbiResult(inferredHiddenStates: List[String], logScore: Double) {
    def InferredHiddenStates: List[String] = inferredHiddenStates
    def LogScore: Double = logScore
  }

  private def viterbiCompute(probs: Map[String,Double],
                             observedStates: Seq[ObservedState]): ViterbiResult = {
    ...
    new ViterbiResult(path.toList, vcur.values.sum)
  }
}
